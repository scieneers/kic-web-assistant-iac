resource "azurerm_postgresql_flexible_server" "this" {
  ...
  # TODO change
  administrator_password = data.sops_file.secrets.data["POSTGRES_PASSWORD"]
  create_mode            = "Default"
  storage_mb             = 32768
  storage_tier           = "P4"
...
resource "azurerm_linux_web_app" "langfuse" {
  ...
  site_config {
    always_on = true
    application_stack {
      docker_image_name   = "langfuse:2.27.0"
  ...
  app_settings = {
    ...
    SALT              = "mysalt"
    NEXTAUTH_URL      = "https://kic-langfuse-${local.environment}.azurewebsites.net"
